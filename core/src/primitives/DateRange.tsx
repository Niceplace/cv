...
export function formatDateRange({
  ...
  const formatDate = (dateStr: string | Date | undefined | null): string => {
    ...
    if (isNaN(date.getTime())) return String(dateStr) // Invalid date, return as-is

    // Configure Intl.DateTimeFormat based on format style
    const monthFormats: Record<
      'short' | 'long' | 'numeric',
...
const StyledDateRange = styled.span<React.HTMLAttributes<HTMLSpanElement>>`
  font-size: ${(props) =>
...
export function DateRange({
  ...
  className,
  ...rest
}: DateRangeProps & React.HTMLAttributes<HTMLSpanElement>) {
  ...
  return (
    <StyledDateRange className={`resume-date-range ${className || ''}`.trim()} {...rest}>
      {formatted}
    </StyledDateRange>
  )
}
