...
const StyledBadge = styled.span<{
  ...
  border-radius: ${(props) => (props.theme as any)?.radius?.sm || 'var(--resume-radius-sm, 4px)'};
  font-size: ${(props) => {
  ...
  background: ${(props) => {
    if (props.$variant === 'accent') {
      return (
        (props.theme as any)?.colors?.accentLight || 'var(--resume-color-accent-light, #e6f2ff)'
      )
    }
    if (props.$variant === 'secondary') {
      return (props.theme as any)?.colors?.muted || 'var(--resume-color-muted, #f5f5f5)'
    }
    return (props.theme as any)?.colors?.muted || 'var(--resume-color-muted, #f5f5f5)' // default
  }};
  color: ${(props) => {
    if (props.$variant === 'accent') {
      return (props.theme as any)?.colors?.accent || 'var(--resume-color-accent, #0066cc)'
    }
    return (props.theme as any)?.colors?.primary || 'var(--resume-color-primary, #000)'
  }};
...
export function BadgeList({
  ...
  className,
  ...rest
}: BadgeListProps & React.HTMLAttributes<HTMLDivElement>) {
  ...
  return (
    <BadgeContainer className={`resume-badge-list ${className || ''}`.trim()} {...rest}>
      {content}
    </BadgeContainer>
  )
}
